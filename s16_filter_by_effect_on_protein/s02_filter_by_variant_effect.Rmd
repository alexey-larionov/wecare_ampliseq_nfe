---
title: "Filter by variant effect"
...
### 1,318 variants x 667 samples -> 164 variants x 667 samples  
...
90 genes in the panel had rare and low-frequency variants.  Of them:  
...
No rare and low-frequency variants were detected in 20 targeted genes (including XPF=ERCC4)  
...
lof_summary_counts.df <- as.data.frame(cbind(
  lof_type=c("splice_acceptor", "splice_donior", "stop_gain", "start_lost", "frameshift", "all_LoFs"),
  lof_count=c(sav.mean, sdv.mean, stogv.mean, stalv.mean, fsv.mean, lof.mean)))
...
lof_summary_counts.df$lof_type <- factor(lof_summary_counts.df$lof_type, 
  levels=c("all_LoFs", "start_lost", "stop_gain", "frameshift", "splice_acceptor", "splice_donior", "stop_lost"))
...
vep_rsf<- genotypes.mx[vep_risk_factor_variant,]
vep_rsf.mean <- mean(vep_rsf, na.rm=TRUE)
vep_rsf.mean
rm(vep_rsf)
...
table(vep_clinsig.counts) # 2 cases with 2 pathogenic clinsig variants per case !
...
Most of selected variants are rare: 162 out of 164  
also the weihgting will further downgrade the variants with AF>0.01  

No variants with AF > 0.95 preserved  
...
- 21 targeted genes had no protein-affecting variants  
...
# Plot the histogram
hist(y$vars_count, main="Histogram for numbers of protein-affecting variants per gene\n(wecare-ampliseq, MAF<0.05)", xlab="Variants per gene", ylab="Number of genes", ylim=c(0,30), breaks=0:11, labels = TRUE)
...
results_file <- paste(base_folder, "s02_selected_variants.txt", sep="/")
