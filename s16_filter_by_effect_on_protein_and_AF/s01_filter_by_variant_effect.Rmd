...
Output: 169 variants x 667 samples

### 1,512 variants x 667 samples -> 169 variants x 667 samples  
...
Then exclude variants that are known benign/likely benign by ClinSig  

Explore remaining variants  

Explore presence of genes of interest (*before filtering by AF*):  
...
genes_107 <- scan(file="/Users/alexey/Documents/wecare/ampliseq/a01_genes_from_EF/candidate_genes_ampliseq_107G_EF_20171122.txt",
              what="character")

source_folder="/Users/alexey/Documents/wecare/ampliseq/v04_ampliseq_nfe/s15_add_PCs_exclude_outliers"
load(paste(source_folder, "s05_exclude_outliers_recalculate_acanaf.RData", sep="/"))
base_folder="/Users/alexey/Documents/wecare/ampliseq/v04_ampliseq_nfe/s16_filter_by_effect_on_protein_and_AF"
rm(source_folder)
...
Variants were detected in 93 genes, 90 of them were expected and 3 - unexpected:  
...
No variants were detected in 17 targeted genes (including REV7=MAD2L2, XPF=ERCC4)  
...
lof_summary_counts.df <- as.data.frame(cbind(
  lof_type=c("all_LoFs","stop_gain","start_lost","frameshift","splice_acceptor","splice_donior"),
  lof_count=c(lof.mean,stogv.mean,stalv.mean,fsv.mean,sav.mean,sdv.mean)))
...
lof_summary_counts.df$lof_type <- factor(lof_summary_counts.df$lof_type, 
  levels=c("all_LoFs", "stop_gain", "start_lost", "frameshift", "splice_acceptor", "splice_donior"))
...
vep_rsf.mx <- genotypes.mx[vep_risk_factor_variant,]
vep_rsf.counts <- apply(vep_rsf.mx, 2, sum, na.rm=TRUE)
vep_rsf.mean <- mean(vep_rsf.counts, na.rm=TRUE)
vep_rsf.mean
rm(vep_rsf.mx,vep_rsf.counts)

vep_rsf.df <- variants.df[vep_risk_factor_variant,]
vep_rsf.df[,c("SYMBOL","Consequence","CLIN_SIG","total_AF")]
rm(vep_rsf.df)
...
lof.mx <- genotypes.mx[lof,]
lof.counts <- apply(lof.mx, 2, sum, na.rm=TRUE)
table(lof.counts) # 12 cases witj multiple LoF variants
lof.mean <- mean(lof.counts, na.rm=TRUE) 
lof.mean
...
table(ddm.counts) # many cases with multiple ddm variants
...
table(clnsig.counts) # 1 case with 2 known pathogenic ClinSig variants
...
summary_counts.df$type <- factor(summary_counts.df$type, 
  levels=c("all_selected", "clnsig", "ddm", "LoF"))
...
also the weihgting will further downgrade the variants with AF>0.05  
...
Only 63 genes retained out of 107  
45 targeted genes had no protein-affecting variants  
1 gene was mixed-up (DCLRE1C was included instead of CDLRE1C)  

Still there are some variants in BRCA1/2 and PALB2  
(after excluding the known pathogenic and benign ones)  

It is worth noting that some of the protein-affecting variants, previously detected in NFE (including variants in FOXM1 and SLC9A3R1) are not detected now.  Most likely this happened because of different variant filtering criteria that had to be applied in Ampliseq, comparatively to previous WES analysis. Similar observation was previously made about the PALB2 carriers, which were detected in Ampliseq-only, but not in Ampliseq-NFE.  
...
genes_63 <- unique(variants.df$SYMBOL)
missed_genes <- setdiff(genes_107, genes_63)
sort(missed_genes)
...
# Plot the histogram
hist(y$vars_count, main="Histogram for numbers of protein-affecting variants per gene\n(wecare-ampliseq, MAF<0.05)", xlab="Variants per gene", ylab="Number of genes", ylim=c(0,40), breaks=c(1:12), labels = TRUE)

# Mean number of variants per gene is slightly higher than in the WES dataset (~4 vs ~2)
mean(y$vars_count)

# Clean-up
rm(x,y,genes_107,genes_63,missed_genes)
...
save.image(paste(base_folder, "s01_filter_by_variant_effect.RData", sep="/"))
