...
###Input data:  
- 162 variants x 664 samples (467BC = 232UBC + 235CBC and 197NFE)  

###Output data:  
- 162 variants x 664 samples  
- 62 genes x 664 samples  

###Overall:
No significantly associated genes after multiple testing correction  

Of the "risk" genes, before the multiple testing:  
- SETX gene stands out for risk because of the difference between NFE and BC  
- BRIP1 gene stands out becaus of the trend NFE<UBC<CBC  

Of the "protective genes"  
- DCHS2 was more present in NFE than in Ampliseq (approaching significance after FDR too)  
...
# Base folder
library(knitr)
base_folder="/Users/alexey/Documents/wecare/ampliseq/v04_ampliseq_nfe/s16_crude_stats"
...
p_obs <- genes.df$ubc_cbc_fisher_p
p_exp <- ppoints(p_obs)
cptn="UBC vs CBC\ncrude counts aggregated per gene\nFisher exact test"
...
# Clean-up
rm(qqunif.plot, p_obs, p_exp, cptn)
...
genes.df[c("ATM","CHEK2"), 
         c("aggr_ac", "aggr_ac_nfe", "aggr_ac_bc", "aggr_ac_ubc", "aggr_ac_cbc","crude_trend_direction")]
...
x <- genes.df %>% 
  select(gene, num_var, multiallelic, aggr_ac, 
         aggr_af_nfe, aggr_af_ubc, aggr_af_cbc, 
         nfe_bc_fisher_p, ubc_cbc_fisher_p,prop_trend_p,
         crude_trend_direction) %>% 
