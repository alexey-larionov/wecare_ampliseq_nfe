...
Input eigenvectors for 715 samples (calculated using 214 non-rare variants not in LD) 

Add eigenvectors to phenotypes, make PCA plots and  
calculate 26 PC outliers: outside of mean +/- 3*sd on top 2 eigenvectors
...
# Eigenvectors
eigenvectors_file <- paste(base_folder, "ampliseq_nfe_214_715_100PCs.eigenvec", sep="/")
eigenvectors.df <- read.table(eigenvectors_file, header=T, sep="\t",quote="")

# Eigenvalues
eigenvalues_file <- paste(base_folder, "ampliseq_nfe_214_715_100PCs.eigenval", sep="/")
...
save.image(paste(base_folder, "s03_add_PCs_214_715.RData", sep="/"))
