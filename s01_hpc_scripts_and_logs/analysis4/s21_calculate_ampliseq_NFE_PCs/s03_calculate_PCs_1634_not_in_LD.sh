...
# s03_calculate_PCs.sh
# Started: Alexey Larionov, 13Jul2019
# Last updated: Alexey Larionov, 29Aug2019

# Use:
# sbatch s03_calculate_PCs.sh

# Calculate PCs for ampliseq-NFE dataset

# ------------------------------------ #
#         sbatch instructions          #
# ------------------------------------ #

#SBATCH -J s03_calculate_PCs
...
#SBATCH --output=s03_calculate_PCs.log
#SBATCH --ntasks=4
#SBATCH --qos=INTR
...
# Start message
echo "Calculate PCs for ampliseq-NFE dataset"
date
echo ""
...
data_folder="${base_folder}/users/alexey/wecare/wecare_ampliseq/analysis4/ampliseq_nfe/data_and_results/s21_calculate_ampliseq_NFE_PCs"

source_vcf_gz="${data_folder}/s02_ampliseq_nfe_1850_715.vcf.gz"
...
# Remove folder with previous results, if any 
plink_dataset_folder="${data_folder}/s01_vcf_to_plink"
rm -fr "${plink_dataset_folder}"
mkdir -p "${plink_dataset_folder}"

# Set the PLINK file-set name
initial_plink_dataset="${plink_dataset_folder}/ampliseq_nfe_1850_715"
...
# --- Update IDs --- #
# Some of the variants in VCF have not had rs-IDs), 
# which led to dots instead of IDs in 2nd column of PLINK BIM files

mv "${initial_plink_dataset}.bim" "${initial_plink_dataset}.bim.old"
awk '{$2=NR"_"$2; print}' "${initial_plink_dataset}.bim.old" > "${initial_plink_dataset}.bim"

# --- Exclude variants in LD --- #

# Output folder
output_data_folder="${data_folder}/s02_exclude_variants_in_LD"
rm -fr "${output_data_folder}"
mkdir -p "${output_data_folder}"

# Output files
pairphase_LD="${output_data_folder}/pairphase_LD"
LD_pruned_datset="${output_data_folder}/ampliseq_nfe_1634_715_not_in_LD"
...
pca_results_folder="${data_folder}/s03_pca"
rm -fr "${pca_results_folder}"
mkdir -p "${pca_results_folder}"
pca_results="${pca_results_folder}/ampliseq_nfe_1634_715_100PCs"
...
echo "Calculated 100 top PCs using 1634 avaialble variants not in LD"
echo ""
